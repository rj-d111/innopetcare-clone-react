...
const ColorDropdown = ({ color, onColorSelect }) => {
  ...
  const handleColorSelect = (color) => {
    ...
    onColorSelect(color); // Call parent callback with the selected color
  };
  ...
  return (
    <div className="relative inline-flex flex-col">
      ...
      >
        {selectedColor ? (
          <>
            ...
            ></span>
            <span>{selectedColor}</span> {/* Display the selected hex code */}
          </>
        ) : (
          "Select Color"
        )}
      ...
      {isOpen && (
        <ul
          className="absolute mt-2 w-[75vw] border border-gray-300 bg-white rounded shadow-lg z-10 grid grid-cols-3 sm:grid-cols-5 md:grid-cols-10 lg:grid-cols-12 gap-2 p-2"
          role="menu"
