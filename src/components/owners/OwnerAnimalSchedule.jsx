...
export default function OwnerAnimalSchedule() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      const appointmentsQuery = query(
        collection(db, "appointments-shelter"),
  ...
  const handleAction = async (appointmentId, status, eventDatetime) => {
    try {
      const appointmentRef = doc(db, "appointments-shelter", appointmentId);
          ...
          <div key={app.id} className="flex justify-between mb-4 p-4 border rounded-md">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p>Client: {clients[app.clientId] || "Unknown"}</p>
                <p>Number of Visitors: {app.numberOfVisitors}</p>
