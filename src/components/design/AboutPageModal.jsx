...
export default function AboutPageModal({ projectId, closeModal, onSectionAdded, selectedSection }) {
  ...
  const uploadImages = async () => {
    ...
    const imageUrls = await Promise.all(
      images.map((image) => {
        const imageRef = ref(storage, `about-sections/${uuidv4()}-${image.name}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedSection) {
      ...
      if (selectedSection) {
        const sectionDocRef = doc(db, "about-sections", projectId, "sections", selectedSection.id);
        await updateDoc(sectionDocRef, sectionData);
        toast.success("Section successfully updated");
      } else {
        const sectionsCollectionRef = collection(db, "about-sections", projectId, "sections");
  ...
  return (
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl relative">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">About Page Section</h2>
